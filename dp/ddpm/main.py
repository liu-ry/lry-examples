...
依赖关系
--------
  shared/model.py          → SimpleUNet（噪声预测网络，与 DDIM/SD 共用）
  shared/noise_schedule.py → NoiseSchedule（β-schedule + 前向加噪 + 训练损失）
...
# 将 dp/ 根目录加入 path，使 shared/ 可被直接 import
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from shared.model         import SimpleUNet
...
def train(epoch: int) -> float:
    ...
    for batch_idx, (imgs, _) in enumerate(train_loader):
        imgs = imgs.to(device)
        optimizer.zero_grad()
        loss = schedule.p_losses(model, imgs)    # ← shared
        loss.backward()
...
def validate(epoch: int) -> float:
    ...
    with torch.no_grad():
        for imgs, _ in val_loader:
            total += schedule.p_losses(model, imgs.to(device)).item()   # ← shared
